...
public class KdTree {
    ...
    private Node insert(Node node, Point2D p, Node parent, boolean vertical) {
        if (node == null) {
            size++;
            RectHV rect;
            if (parent == null) {
        ...
        if (vertical) {
            ...
            if (cmp < 0) {
                node.lb  = insert(node.lb, p, node, false);
            }
            else if (cmp > 0 || (cmp == 0 && p.y()- node.p.y() != 0)) {
                node.rt = insert(node.rt, p, node, false);
            }
        }
        else {
            double cmp = p.y()- node.p.y();
            if (cmp < 0) {
                node.lb  = insert(node.lb, p, node, true);
            }
            else if (cmp > 0 || (cmp == 0 && p.x()- node.p.x() != 0)) {
                node.rt = insert(node.rt, p, node, true);
            }
        }
        return node;
    }
