...
fun eval_prog (e,env) =
	...
	(case List.find (fn (s2,v) => s=s2) env of
	  ...
	  | Shift(deltaX, deltaY, e0) => (case e0 of
	  						NoPoints => e0
							  | Point(x, y) => Point(x + deltaX, y + deltaY)
							  | Line(m, b) => Line(m, b + deltaY - m * deltaX)
							  | VerticalLine(x) => VerticalLine(x + deltaX)
							  | LineSegment(a, b, c, d) => LineSegment(a + deltaX, b + deltaY, c + deltaX, d + deltaY)
							  | _ => eval_prog(Shift(deltaX, deltaY, eval_prog(e0, env)), env)
	  						)
...
fun preprocess_prog(e) = 
	case e of
		LineSegment(a,b,c,d) => if real_close_point (a, b) (c, d)
								then Point(a, b)
								else if (not (real_close (a, c)) andalso a > c) orelse  (real_close (a, c) andalso b > d)
		...
		| Shift(deltaX, deltaY, e0) => Shift(deltaX, deltaY, preprocess_prog(e0))
		| _ => e