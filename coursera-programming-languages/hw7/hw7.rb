...
class Point < GeometryValue
  ...
  def inbetween(v,end1,end2)
    ...
    (end1 - eps <= v && v <= end2 + eps) || (end2 - eps <= v && v <= end1 + eps)
  end
...
class LineSegment < GeometryValue
  ...
  def intersect2VerticalSegments(seg1, seg2)
    if real_close(seg1.y2,seg2.y1)
      Point.new(seg1.x2,seg1.y2) #just touching
    elsif seg1.y2 < seg2.y1
      NoPoints.new #disjoint
    elsif seg1.y2 > seg2.y2
      seg2 #(* b inside a *)
    else 
      LineSegment.new(seg2.x1,seg2.y1,seg1.x2,seg1.y2) #(* overlapping *)
    end
  end

  def intersect2Segments(seg1, seg2)
  ...
  def intersectWithSegmentAsLineResult seg
      ...
      @y1 < seg.y1 ? intersect2VerticalSegments(self, seg) : intersect2VerticalSegments(seg, self)
    else
      @x1 < seg.x1 ? intersect2Segments(self, seg) : intersect2Segments(seg, self)
    end
  end
end
