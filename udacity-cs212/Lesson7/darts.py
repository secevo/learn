...
setS = set(range(1,21)+[25])
setD = set([2*i for i in range(1,21)] + [50])
setT = set([3*i for i in range(1,21)])
listAll = [0] + sorted(list(setS | setD | setT), reverse=True)
...
def double_out(total):
    ...
    try:
        darts =  next((d1,d2,total-d1-d2) for d1 in listAll for d2 in listAll if total-d1-d2 in setD) 
    except:
        return None
    results = []
    for d in darts:
        if d > 0:
            if d in setS: 
                results.append('S'+('B' if d==25 else str(d)))
            elif d in setD: 
                results.append('D'+('B' if d/2==25 else str(d/2)))
            elif d in setT:
                results.append('T'+ str(d/3))
    return results 
...
def ring_outcome(target, miss):
    if target[0]=='T':
        ...
        return {'DB':1-miss*3, 'SB':miss, 'S':miss*2}
    elif target[0]=='D':
        return {'D':1-miss, 'S':miss/2, 'OFF':miss/2}
    elif target=='SB':
        return {'SB':1-miss, 'DB':miss*0.25, 'S':miss*0.75}
    elif target[0]=='S':
        return {'S':1-miss/5, 'T':miss/10, 'D':miss/10}
    
def section_outcome(target, miss): 
    if target=='DB' or target=='SB':
        results = {'B':1-miss}
        for section in sections:
            results[str(section)] = miss/20
    else:
        section = target[1:]
        i = sections.index(int(section))
        left = sections[(i-1)%20]
        right = sections[(i+1)%20]
        results = {section:1-miss, str(left):miss/2, str(right):miss/2}
    return results

def outcome(target, miss):
    "Return a probability distribution of [(target, probability)] pairs."
    results =defaultdict(float)
    ringD = ring_outcome(target, miss)
    sectionD = section_outcome(target, miss)
    for ring in ringD:
        for section in sectionD:
            if ring=='S' and section=='B':
                for i in sections:
                    results[Target(ring ,str(i))] += ringD[ring]*sectionD[section]/20
            else:
                results[Target(ring ,section)] += ringD[ring]*sectionD[section]
    return results
...
def best_target(miss):
    ...
    all_target = [ring+str(section) for ring in ('S','D','T') for section in range(1,21)] + ['SB', 'DB']
    return max(all_target, key=lambda target: expect_socre(outcome(target, miss)))
...
def expect_socre(distribution):
    return sum(socre(target)*distribution[target] for target in distribution)

def socre(target):
    if target=='OFF': 
        return 0
    ring = target[0]
    mul = 1 if ring=='S' else 2 if ring=='D' else 3
    section = 25 if target[-1]=='B' else int(target[1:])
    return mul*section
