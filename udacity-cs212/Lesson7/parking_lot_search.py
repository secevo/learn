...
def solve_parking_puzzle(start, N=N):
    ...
    return shortest_path_search(start, lambda s:successors(s, N), lambda s:is_goal(s, N))
    
def successors(state, N):
    succ = dict()
    b = board(state, N)
    for car, locations in state:
        if car!= '@' and car!= '|':
            d = direction(locations)
            left, right = max_move(b, locations, d)
            for i in range(-left,0) + range(right,0,-1):
                succ[move(state, car, i*d)] = (car, i*d)         
    return succ

def move(state, car, shifting):
    l = list(state)
    i, ps = next((i, ps) for i, (c, ps) in enumerate(l) if c==car)
    l[i] = (car, tuple(p + shifting for p in ps))
    return tuple(l)

H, V = 1, 8 

def direction(locations):
    return locations[1]-locations[0]

def max_move(board, locations, d):
    i, j = 0, 0
    while board[locations[0]-d*(i+1)] in ('.', '@'):i += 1
    while board[locations[-1]+d*(j+1)] in ('.', '@'):j += 1
    return (i, j)
    
def is_goal(state, N):
    goal = (N+1)/2*N - 1
    stars = dict(state)['*']
    return goal in stars
...
def grid(cars, N=N):
    ...
    for row in range(1,N-1):
        walls += [row*N, row*N+N-1]
    walls += range((N-1)*N, N*N)
    walls.remove(goal)
    return (('@', (goal,)),) + cars + (('|', tuple(walls)),)
...
def show(state, N=N):
    ...
    # Now print it out
    for i,s in enumerate(board(state, N)):
        print s,
        if i % N == N - 1: print
...
def my_test():
 ...
 ('|', (0, 1, 2, 3, 4, 5, 6, 7, 8, 15, 16, 23, 24, 32, 39,
        40, 47, 48, 55, 56, 57, 58, 59, 60, 61, 62, 63)))
    assert is_goal(puzzle_goal, 8)
    my_board = board(puzzle_goal)
    assert max_move(my_board, (9, 10), H)== (0, 3)
    assert max_move(my_board, (14, 22, 30), V)==(0, 1)
    assert max_move(my_board, (45, 46), H)==(3,0)
    assert max_move(my_board, (26, 27), H)==(0,0)
    show(puzzle1)
    print list(successors(puzzle1, 8).values())
    path = solve_parking_puzzle(puzzle1, N=8)
